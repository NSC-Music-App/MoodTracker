...
//อารมณ์ที่จะให้เลือก
export type MoodType = 'happy' | 'sleepy' | 'sad' | 'angry' | 'Mind\nBlowing';
...
const CalendarScreen = () => {
    ...
    const handleMoodSelection = (mood: MoodType) => {
      if (selectedDate) {
        setMoods(prev => ({
          ...prev, 
          [selectedDate]: { mood, color: MOOD_COLORS[mood] }
        }));
    ...
    const handleDatePress = (date: Date) => {
        // Allow selecting any date that is not in the future
    ...
    return (
        <View style={styles.container}>
          ...
          {showMoodPicker && (
            <MoodPicker onSelect={handleMoodSelection} />
          )}
        </View>
      );
};
